...
describe('Edge Cases', () => {
  ...
  describe('Language Detection Edge Cases', () => {
    ...
    it('should handle language case insensitivity', async () => {
      const highlighter = await createHighlighter()

      // These should all work via aliases
      const result1 = await highlighter.highlight('const x = 1;', 'JavaScript')
      const result2 = await highlighter.highlight('const x = 1;', 'JAVASCRIPT')
      const result3 = await highlighter.highlight('const x = 1;', 'js')
